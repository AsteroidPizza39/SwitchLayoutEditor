...
namespace BflytPreview
{
	...
	/// Inverse maps a desired appearance back to the file color to store.
	/// </summary>
	internal static class LayoutDisplayColor
	{
		...
		public static float InverseLiftChannel(float c01) =>
			(float)Math.Pow(Math.Max(Math.Min(c01, 1f), 0f), Gamma);

		public static byte LiftByte(byte c) =>
			(byte)Math.Max(0, Math.Min(255, (int)Math.Round(LiftChannel(c / 255f) * 255f)));

		public static byte InverseLiftByte(byte c) =>
			(byte)Math.Max(0, Math.Min(255, (int)Math.Round(InverseLiftChannel(c / 255f) * 255f)));

		public static RGBAColor Lift(RGBAColor c) =>
			new RGBAColor(LiftByte(c.R), LiftByte(c.G), LiftByte(c.B), c.A);

		public static RGBAColor InverseLift(RGBAColor appearance) =>
			new RGBAColor(
				InverseLiftByte(appearance.R),
				InverseLiftByte(appearance.G),
				InverseLiftByte(appearance.B),
				appearance.A);

		public static Color Lift(Color c) =>
			Color.FromArgb(c.A, LiftByte(c.R), LiftByte(c.G), LiftByte(c.B));

		public static Color InverseLift(Color appearance) =>
			Color.FromArgb(
				appearance.A,
				InverseLiftByte(appearance.R),
				InverseLiftByte(appearance.G),
